...
def _is_verbose_debug(opt):
    value = getattr(opt, 'debug_verbose', 'no')
    return str(value).strip().lower() in ('1', 'true', 'yes', 'on')


def _debug_log(logger, opt, event, **kwargs):
    if logger and _is_verbose_debug(opt):
        payload = {'debug_event': event}
...
async def toc_transformer(toc_content, model=None, semaphore=None):
    ...
    print(f"toc_transformer initial finish_reason={finish_reason}, raw_preview={str(last_complete)[:500]}")
    if_complete = check_if_toc_transformation_is_complete(toc_content, last_complete, model)
    if if_complete == "yes" and finish_reason == "finished":
        parsed_last_complete = extract_json(last_complete)
        if isinstance(parsed_last_complete, dict) and isinstance(parsed_last_complete.get('table_of_contents'), list):
            cleaned_response = convert_page_to_int(parsed_last_complete['table_of_contents'])
            return cleaned_response
        if isinstance(parsed_last_complete, list):
            cleaned_response = convert_page_to_int(parsed_last_complete)
            return cleaned_response
        print(f"toc_transformer parse fallback failed after finished response: {type(parsed_last_complete).__name__}")
    ...
    print(
        f"toc_transformer failed to parse final response for model={model}; "
        f"returning empty toc. raw_preview={str(last_complete)[:500]}"
    )
    return []
...
def _continue_json_generation_until_complete(response, base_prompt, model=None, max_attempts=5):
    """
    Continue a truncated JSON-style generation until the model stops hitting the output limit.

    This is used for TOC generation where the model may emit a partial JSON array and stop with
    `max_output_reached`. The helper asks the model to continue from the previous assistant output
    and appends each new chunk until it can be parsed or the retry budget is exhausted.
    """
    if not response:
        return response
    ...
    attempt = 0
    continuation_prompt = (
        "Continue from the previous assistant response. "
        "Return only the remaining JSON items and do not repeat items that were already emitted. "
    ...
    while finish_reason == "max_output_reached" and attempt < max_attempts:
        attempt += 1
        ...
        if parsed_response:
            # If the model produced a parseable JSON payload, return the accumulated response.
            # The caller can parse it again and normalize types.
            break
...
def generate_toc_continue(toc_content, part, model=None):
    ...
    if finish_reason != 'finished':
        response = _continue_json_generation_until_complete(response, prompt, model=model)
...
def _normalize_toc_items(toc_data, logger=None, context="toc"):
    """Normalize varied LLM outputs into a strict list[dict] for downstream processing."""
    if toc_data is None:
        return []
    ...
    if isinstance(candidate, dict):
        if isinstance(candidate.get("table_of_contents"), list):
            candidate = candidate["table_of_contents"]
        elif isinstance(candidate.get("toc"), list):
            candidate = candidate["toc"]
        elif all(k in candidate for k in ("title", "physical_index")):
            candidate = [candidate]
        else:
            list_value = next((v for v in candidate.values() if isinstance(v, list)), None)
            candidate = list_value if list_value is not None else []

    if not isinstance(candidate, list):
        if logger:
            logger.warning(f"Unexpected TOC payload type in {context}: {type(toc_data).__name__}")
        return []

    normalized = [item for item in candidate if isinstance(item, dict)]
    if logger and len(normalized) != len(candidate):
        logger.warning(
            f"Filtered non-dict TOC items in {context}: kept {len(normalized)}/{len(candidate)}"
        )
    return normalized
...
async def process_toc_no_page_numbers(toc_content, toc_page_list, page_list,  start_index=1, model=None, logger=None, semaphore=None):
    page_contents=[]
    token_lengths=[]
    toc_content = _normalize_toc_items(
        await toc_transformer(toc_content, model, semaphore=semaphore),
        logger=logger,
        context="process_toc_no_page_numbers_transformer",
    )
    ...
    for group_text in group_texts:
        toc_with_page_number = _normalize_toc_items(
            await add_page_number_to_toc(group_text, toc_with_page_number, model, semaphore=semaphore),
            logger=logger,
            context="process_toc_no_page_numbers_add_page",
        )
...
async def process_toc_with_page_numbers(toc_content, toc_page_list, page_list, toc_check_page_num=None, model=None, logger=None, semaphore=None):
    toc_with_page_number = _normalize_toc_items(
        await toc_transformer(toc_content, model, semaphore=semaphore),
        logger=logger,
        context="process_toc_with_page_numbers_transformer",
    )
    ...
    toc_with_physical_index = _normalize_toc_items(
        toc_index_extractor(toc_no_page_number, main_content, model),
        logger=logger,
        context="process_toc_with_page_numbers_extractor",
    )
...
async def check_toc(page_list, opt=None, doc=None, logger=None, semaphore=None):
    _debug_log(
        ...
        toc_page_range=getattr(opt, 'toc_page_range', None),
        toc_check_page_num=getattr(opt, 'toc_check_page_num', None),
    )
    outline_toc = extract_toc_from_pdf_outline(doc, logger=logger) if doc is not None else []
    if outline_toc:
        print('outline found, use outline toc directly')
        if logger:
            logger.info({'toc_source': 'outline', 'toc_item_count': len(outline_toc)})
            logger.info({'toc_source': 'outline', 'toc_items': outline_toc})
        return {
            'toc_with_page_number': outline_toc,
            'toc_source': 'outline',
            'page_index_given_in_toc': 'yes'
        }
    print("no outline toc found, start to find toc from page content")
    ...
    if toc_page_list_by_range is not None:
        _debug_log(
            logger,
            opt,
            'check_toc_range_mode',
            start_page=(toc_page_list_by_range[0] + 1) if toc_page_list_by_range else None,
            end_page=(toc_page_list_by_range[-1] + 1) if toc_page_list_by_range else None,
            selected_pages=len(toc_page_list_by_range),
        )
    ...
    _debug_log(logger, opt, 'check_toc_auto_find_result', toc_pages=toc_page_list, found_count=len(toc_page_list))
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None, semaphore=None):
    ...
    _debug_log(
        logger,
        opt,
        'meta_processor_start',
        mode=mode,
        start_index=start_index,
        page_count=len(page_list),
        has_toc_content=bool(toc_content),
        toc_page_count=len(toc_page_list) if toc_page_list else 0,
    )
    ...
    toc_with_page_number = _normalize_toc_items(
        ...
        logger=logger,
        context=f"meta_processor_{mode}",
    )
    _debug_log(
        logger,
        opt,
        'meta_processor_after_normalize',
        mode=mode,
        toc_item_count=len(toc_with_page_number),
    )
    toc_with_page_number = [item for item in toc_with_page_number if item.get('physical_index') is not None]
    _debug_log(
        logger,
        opt,
        'meta_processor_after_physical_index_filter',
        mode=mode,
        toc_item_count=len(toc_with_page_number),
    )
    ...
    if accuracy > 0.6 and len(incorrect_results) > 0:
        ...
        if mode == 'process_toc_with_page_numbers':
            ...
            logger.warning(
                "TOC verification did not pass in process_no_toc; "
                "returning current TOC as fallback to keep pipeline running."
            )
            return toc_with_page_number
...
async def process_large_node_recursively(node, page_list, opt=None, logger=None, semaphore=None):
    ...
    _debug_log(
        ...
        page_span=(node.get('end_index', 0) - node.get('start_index', 0)),
        token_num=token_num,
        max_page_num_each_node=getattr(opt, 'max_page_num_each_node', None),
        max_token_num_each_node=getattr(opt, 'max_token_num_each_node', None),
    )
    ...
    if node['end_index'] - node['start_index'] > opt.max_page_num_each_node and token_num >= opt.max_token_num_each_node:
        ...
        _debug_log(
            logger,
            opt,
            'large_node_split_result',
            node_id=node.get('node_id'),
            title=node.get('title'),
            split_item_count=len(node_toc_tree) if node_toc_tree else 0,
        )
...
def parse_toc_page_range(toc_page_range, total_pages):
    ...
    if not match:
        raise ValueError(
            f"Invalid toc_page_range format: '{toc_page_range}'. "
            "Use format like '3-8' (string). "
            "If setting in Python dict, write \"10-15\" not 10-15."
        )
...
async def tree_parser(page_list, opt, doc=None, logger=None):
    ...
    if check_toc_result.get('toc_source') == 'outline':
        ...
        _debug_log(logger, opt, 'tree_parser_mode_selected', mode='process_toc_with_page_numbers')
        ...
        _debug_log(logger, opt, 'tree_parser_mode_selected', mode='process_no_toc')
        toc_with_page_number = await meta_processor(
            page_list, 
            mode='process_no_toc', 
            start_index=1, 
    ...
    _debug_log(
        logger,
        opt,
        'tree_parser_post_filter',
        valid_toc_item_count=len(valid_toc_items),
    )
...
def page_index_main(doc, opt=None, progress_callback=None):
    ...
    _debug_log(
        logger,
        opt,
        'page_index_main_start',
        doc_name=get_pdf_name(doc),
        model=getattr(opt, 'model', None),
        debug_verbose=getattr(opt, 'debug_verbose', 'no'),
        toc_page_range=getattr(opt, 'toc_page_range', None),
        toc_check_page_num=getattr(opt, 'toc_check_page_num', None),
    )
...
def page_index(doc, model=None, toc_check_page_num=None, max_page_num_each_node=None, max_token_num_each_node=None,
               ...
               toc_page_range=None, llm_max_concurrency=None, if_add_page_labels=None, debug_verbose=None):
