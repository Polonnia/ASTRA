...
def normalize_extracted_field(input: Any) -> str:
    """Normalize model-extracted field text by removing wrapper/dangling quotes and control chars."""
    if not isinstance(input, str):
        return input

    result = clean_str(input).strip()

    # Remove one or two layers of surrounding quotes.
    for _ in range(2):
        if len(result) >= 2 and (
    ...
    # Remove a single dangling quote at either end.
